...
var homeapp = angular.module("homeapp",['ngCookies']).config(function($anchorScrollProvider) {
    $anchorScrollProvider.disableAutoScrolling();
});

homeapp.factory('LANGUAGE', function(){
    return {Language:null};
});

homeapp.factory("COOKIE_SAVE",function(){
...
homeapp.controller('topMenuController',topMenuController);
...
homeapp.controller('bottomMenuController',bottomMenuController);
...
function middleContextController($scope, $rootScope, $cookieStore, $http, $q, LANGUAGE, COOKIE_SAVE,CURRENT_NETWORK){
    ...
    function getAccountInfo(ind,accountAddr){
        ...
        $http.get(urlget)
            .success(function (data,header,config,status){
                ...
                $scope.innerContexts[ind].refreshBtnIcon = "fa fa-refresh";
    ...
    function initPaymentView(ind){
        $scope.innerContexts[ind].paymentInfo.btnStatus = "fa fa-check";
    ...
    function initMergeView(ind){
        $scope.innerContexts[ind].mergeInfo.btnStatus = "fa fa-check";
    ...
    function quaryExecute(ind, mainAddr, quaryCount, isNext){
        ...
        $http.get(urlget)
            .success(function (data,header,config,status){
                ...
                $scope.innerContexts[ind].quaryInfo.btnStatus = "fa fa-search";
    ...
    function getInnerContext(i,a){
        ret = {
            ...
            address:a,
            refreshBtnIcon:"fa fa-refresh",
            refreshClick : function(ind){
                if (this.refreshBtnIcon == WAIT_ICON_STATE){
                    return;
                }
                //console.log("refreshClick ============");
                this.refreshBtnIcon = WAIT_ICON_STATE;
            ...
            paymentClick : function(ind) {
                if (this.mergeInfo.btnStatus == WAIT_ICON_STATE ||
                    this.paymentInfo.btnStatus == WAIT_ICON_STATE ||
                    this.quaryInfo.btnStatus == WAIT_ICON_STATE )
                    return;
                this.paymentInfo.btnStatus = WAIT_ICON_STATE;
...
homeapp.controller('middleContextController',middleContextController);
